...
bool GLTexture::Load(char* name) {
	...
	if (strstr(texturename, ".bmp")) {
		LoadBMP(texturename);
		return true; // Assume success (modify LoadBMP to return false if it fails)
	}
	else if (strstr(texturename, ".tga")) {
		LoadTGA(texturename);
		return true; // Assume success (modify LoadTGA to return false if it fails)
	}
	else if (strstr(texturename, ".png")) {
		LoadPNG(texturename);
		return true;// Implement LoadPNG with a success/failure return
	}
...
void GLTexture::LoadPNG(char* name) {
	int width, height, channels;
	unsigned char* data = stbi_load(name, &width, &height, &channels, 4); // Force RGBA
