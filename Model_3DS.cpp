...
void Model_3DS::DiffuseColorChunkProcessor(long length, long findex, int matindex)
{
	...
	// Store the original color after processing
	Materials[matindex].originalColor = Materials[matindex].color;

	fseek(bin3ds, findex, SEEK_SET);
}

// Implement the SetMaterialBrightness method
void Model_3DS::SetMaterialBrightness(int matIndex, float brightnessFactor)
{
	// Ensure brightnessFactor is positive
	if (brightnessFactor < 0.0f) brightnessFactor = 0.0f;

	// Increase the brightness by multiplying the color components
	Materials[matIndex].color.r = (std::min)(static_cast<int>(Materials[matIndex].originalColor.r * brightnessFactor), 255);
	Materials[matIndex].color.g = (std::min)(static_cast<int>(Materials[matIndex].originalColor.g * brightnessFactor), 255);
	Materials[matIndex].color.b = (std::min)(static_cast<int>(Materials[matIndex].originalColor.b * brightnessFactor), 255);
	// Alpha remains unchanged
	Materials[matIndex].color.a = Materials[matIndex].originalColor.a;
}

// Implement the ResetMaterialColor method
void Model_3DS::ResetMaterialColor(int matIndex)
{
	Materials[matIndex].color = Materials[matIndex].originalColor;
}
